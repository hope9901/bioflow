...
def align(clean, bowtie2_index: Path, sample_id: str, *, out_dir):
    ...
    ``biocontainers/bowtie2`` image does not).  Pinned to 2.5.1: its 2.5.4/2.5.5
    tags ship a samtools that can't load its shared libs (libdeflate /
    libcurl-gnutls), which would break this ``bowtie2 | samtools sort`` chain.  Trimmed-read filenames
